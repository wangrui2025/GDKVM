...
function initDependentSelects() {
  ...
  const nValue = document.getElementById('nValue') as HTMLSelectElement | null;
  const naValue = document.getElementById('naValue') as HTMLSelectElement | null;
  const dValue = document.getElementById('dValue') as HTMLSelectElement | null;
...
function initVisualization() {
  ...
  generateBtn.addEventListener('click', () => {
    // 获取参数值
    const modelTypeEl = document.getElementById('modelType') as HTMLSelectElement | null;
    const nValueEl = document.getElementById('nValue') as HTMLSelectElement | null;
    const dValueEl = document.getElementById('dValue') as HTMLSelectElement | null;
    ...
    if (modelType === 'Dense') {
      ...
      const naValueEl = document.getElementById('naValue') as HTMLSelectElement | null;
    ...
    img.onerror = () => {
      loading.style.display = 'none';
      (errorMsg as HTMLElement).style.display = 'flex';
      container.style.minHeight = '500px';
    };
...
function calculateBsLr(modelSize: number, trainingTokens: number) {
  /**
   * 公式：
...
document.addEventListener('astro:page-load', () => {
  ...
  if (modelForm) {
    modelForm.addEventListener('submit', function (e) {
      ...
      const modelSizeInputEl = document.getElementById('modelSize') as HTMLInputElement | null;
      const trainingTokensInputEl = document.getElementById('trainingTokens') as HTMLInputElement | null;
